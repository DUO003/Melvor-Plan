...
func equipped(_装备栏: String) -> void:
	...
	if 分类 == "护甲":
		# 加入减伤来源数组
		初始化.玩家单例.减伤来源.append(self)
		# 触发减伤计算（在装备脚本中完成）
	计算玩家减伤()
	if 分类 == "武器":
		初始化.玩家单例.攻速=定义攻速()
		初始化.玩家单例.攻击距离=定义攻击距离()
...
func unequipped(_装备栏: String) -> void:
	...
	if 分类 == "护甲":
		# 从减伤来源中移除
		if 初始化.玩家单例.减伤来源.has(self):
			初始化.玩家单例.减伤来源.erase(self)
		# 重新计算减伤
	计算玩家减伤()
	if 分类 == "武器":
		初始化.玩家单例.攻速=初始化.玩家单例.默认攻速
		初始化.玩家单例.攻击距离=初始化.玩家单例.默认攻击距离
# 核心：计算玩家当前总减伤（在装备脚本中实现）
func 计算玩家减伤() -> void:
	# 1. 获取玩家单例
	var 玩家 = 初始化.玩家单例
	if not 玩家:
	...
	# 2. 基于默认减伤初始化承受伤害比例（0-1范围）
	var 承受伤害比例 = 1.0 - (玩家.默认减伤 / 100.0)

	# 3. 循环所有减伤来源（护甲装备）叠加计算
	for 装备 in 玩家.减伤来源:
		# 安全检查：装备有效且是护甲分类、有减伤值方法
		if 装备 == null:
			continue  # 跳过无效装备
		if 装备.分类 != "护甲" or not 装备.has_method("定义减伤"):
			continue  # 只处理有效护甲装备

		# 获取装备减伤值并限制范围
		var 装备减伤 = 装备.定义减伤()
		装备减伤 = clamp(装备减伤, 0, 100)

		# 叠加计算：承受伤害 = 现有比例 × (1 - 装备减伤比例)
		承受伤害比例 *= (100.0 - 装备减伤) / 100.0

	# 4. 计算最终减伤（1 - 承受伤害比例）并赋值给玩家
	玩家.减伤 = 1.0 - 承受伤害比例
	print("玩家减伤",玩家.减伤)
