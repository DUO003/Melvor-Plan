...
func 物品类型可用性检查()->bool:
	var 使用属性:String=计划.表格.获取属性(item_name,"使用","") as String
	return 物品使用映射.has(使用属性)
## 物品被使用时调用,自行处理销毁逻辑与变量外观
func 使用物品(背包) -> String:# 中间函数：处理物品使用流程
	print("尝试使用物品",item_name)
	if not 物品类型可用性检查():# 步骤2：检查字典中是否有对应处理方法
		计划.语法糖通知("该物品没有使用方法","物品使用")
		return "缺少方法"
	var 使用属性:String=计划.表格.获取属性(item_name,"使用","") as String
	var 使用数量 = 物品使用映射[使用属性].call()# 步骤3：执行对应使用方法，获取使用数量
	if 使用数量 == -1:
		print("条件错误")
		return "条件错误"
	else:
...
func 使用方法_礼盒():
	if 数量 >= 1:
		var 字典:Dictionary = 计划.表格.获取属性(item_name, null, {}) as Dictionary
		if 计划.节点有效性检查("主容器窗口"):
			计划.节点["主容器窗口"].便利摄像机效果()
...
func 使用方法_药水():
	if not 计划.BUFF.BUFF配置字典.has(item_name):
		return -1 # 没有药水的BUFF状态
	if 数量 >= 1:
		if 计划.BUFF.创建BUFF(item_name, "使用药水", 1.0):
			计划.语法糖通知(item_name+"使用成功")
			return 1
