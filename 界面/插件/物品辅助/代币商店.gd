...
func 更新文本标签():
	for 文本标签 in 文本标签字典:
		...
		if 额外商品数据.has(类型):
			...
			文本标签.text="%s%s\r%s点数:%d"%[类型,额外数据,点数类型,计划.点数.查看点数(点数类型)]
		else :文本标签.text=类型
		...
		for 方向 in 研究方向:
			...
			for 蓝图 in 蓝图数组:
				...
				if 商品数据资源.has(蓝图):
					...
					克隆数据=数据包.duplicate()
					蓝图费用=int(计划.表格.获取属性(蓝图,"蓝图",-1))
					#print("%s蓝图费用%d"%[蓝图,蓝图费用])
				克隆数据.商品名=蓝图
				克隆数据.费用=int(蓝图费用)
				var 解锁:bool=计划.手工.数据合成配方(蓝图,"解锁")
				if 蓝图费用<=0 or 克隆数据.限购==0 or 解锁:
					克隆数据.限购=0
					添加成功+=1
				else :克隆数据.限购=1
				if 蓝图费用<=0 and not 解锁:克隆数据.额外通知="\n需要抽取"
				else :克隆数据.额外通知=""
				商品数组.append(克隆数据)
				商品数据资源[蓝图]=克隆数据
