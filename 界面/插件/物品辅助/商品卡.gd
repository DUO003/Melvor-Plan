...
func 接触反馈(启用:bool):
	if 启用:
		...
		计划.全局悬浮提示.emit("商品:%s*%d(%s)%s\n价格:%d %s(%s)\n期望购买数量:%d\n拥有代币数量:%d"%[
			商品.商品名,商品.商品数量,商品.商品类型,商品.额外通知,商品.费用,商品.代币,商品.代币类型,
			购买数量,商品.检查背包代币()],self,30)
	else :
...
func 购买逻辑():
	...
	#var 背包内代币
	#match 商品.代币类型:
		#"点数":背包内代币=计划.点数.查看点数(商品.代币)
		#"物品",_:背包内代币=计划.检查背包物品数量(商品.代币)
	#if 背包内代币>=商品.费用*购买数量:
		#if 计划.表格.蓝图标签检查(商品.商品名,"催化"):
			#计划.steam.解锁成就("催化剂兑换")
		#match 商品.代币类型:
			#"点数":背包内代币=计划.点数.消耗点数(商品.代币,-购买数量*商品.费用)
