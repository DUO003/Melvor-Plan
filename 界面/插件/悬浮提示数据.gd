...
func 解析装备栏(资源:EquipmentSlotData,参数:Dictionary={}):
	...
	var 倍率:float=资源.倍率()
	提示数组.append({"文本":"[center][font_size=%d]%s[/font_size][/center]"%[默认字体+10,tr(资源.slot_name)]})
	if 物品:解析装备(物品,{"倍率":倍率,"简化":true})
	else :提示数组.append({"文本":tr("<空的装备栏>")})
...
func 解析物品(资源:标准物品,参数:Dictionary={}):
	提示数组.append({"文本":"[center][font_size=%d]%s[/font_size][/center]"%[默认字体+10,资源.显示名称]})
	if GBIS.shop_names.has(参数.get("背包名","")):##商品信息
		文本数组.clear()
		文本数组.append(tr("<每次购买数量>")% [数值色, 资源.数量])
		文本数组.append(tr("<购买价格>")% [数值色, 资源.价值, 数值色, (资源.价值 * 1.0 / 资源.数量)])
		文本数组.append(tr("<剩余购买次数>")% [数值色, 资源.商店剩余数量])
		# 最终合并（和你原来的换行格式完全一致）
		提示数组.append({"文本": "\n".join(文本数组)})
		提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
	提示数组.append({"模式":"分栏","间距":20,
		"分栏组":[tr("<背包数量>")%[数值色,计划.检查背包物品数量(资源.item_name)],
		tr("<阶级_颜色>")%[数值色,计划.罗马数字(资源.阶级)]]})
	var 翻译标签:Array=[]
	for 标签 in 资源.标签:
		翻译标签.append(tr("<%s>"%标签))
	提示数组.append({"文本":tr("<物品标签>")%[数值色,(",".join(翻译标签))]})
	var 简化:bool=参数.get("简化",false)
	if not 简化:通用提示(计划.表格.蓝图数据(资源.item_name,"价值"),资源.简介,
		{"样式":{"移动":["鼠标右键"],"选中":["鼠标左键"],"拆分":["鼠标中键"]}})
func 解析方块(资源:物品方块, 参数:Dictionary={}):
	提示数组.append({"文本": "[center][font_size=%d]%s[/font_size][/center]" % [默认字体+10, 资源.item_name]})
	文本数组.clear()
	文本数组.append(tr("<堆叠上限_格式>")% [数值色, 资源.堆叠上限])
	文本数组.append(tr("<方块尺寸>")% [数值色, 资源.columns, 资源.rows])
	提示数组.append({"文本": "\n".join(文本数组)})
	var 简化:bool=参数.get("简化",false)
	if not 简化:通用提示(0,资源.简介,{"样式":{"移动":["鼠标右键"],"选中":["鼠标左键"],"拆分":["鼠标中键"]}})
func 解析装备(资源:物品装备,参数:Dictionary={}):
	...
	提示数组.append({"模式":"分栏","间距":20,
		"分栏组":[tr("<装备等级>")%[数值色,资源.等级,资源.阶级*5],tr("<阶级>")%[计划.罗马数字(装备阶级)]]})
	提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
	提示数组.append({"文本": "[font_size=%d]%s[/font_size]" % [默认字体+2,tr("【装备属性】")]})
	var 分类=资源.分类
	...
	if ["武器"].has(分类):
		提示数组.append({"模式":"分栏","间距":10,"分栏组":[
			tr("<攻击倍率>")%[数值色,(字典["攻击"])],
			tr("<攻击类型>")%[数值色,(字典["攻击类型"])]]})
		提示数组.append({"文本":tr("<攻速>")%[数值色,(字典["攻速"]*100)]})
	elif 字典.has("攻击"):
		提示数组.append({"文本":tr("<攻击倍率百分比>")%[数值色,(字典["攻击"]*100)]})
	if 字典.has("减伤"):
		提示数组.append({"文本":tr("<护甲减伤>")%[数值色,(字典["减伤"]*100)]})
	if 字典.has("血量"):
		提示数组.append({"文本":tr("<生命倍率>")%[数值色,(字典["血量"]*100)]})
	if 字典.has("魔法"):
		提示数组.append({"文本":tr("<魔法倍率>")%[数值色,(字典["魔法"])]})
	var 基础属性:Dictionary=资源.基础属性
	for 属性名 in 基础属性:
		提示数组.append({"模式":"分栏","间距":0,
		"分栏组":[(基础属性[属性名]/资源.当前最大属性(属性名)),
		tr("<通用属性加成>")%[属性名,数值色,基础属性[属性名]*倍率]]})
...
func 解析宝石(资源:物品宝石, 参数:Dictionary={}):
	...
	if 简化:
		var 核心文本:String = tr("<宝石倍率>")% [数值色, 倍率值]
		提示数组.append({"模式":"分栏","间距":0,"分栏组": [资源.item_name,资源.数值,核心文本]})
	else:
		var 核心文本:String = tr("<增强装备栏>")% [数值色, 倍率值]
		提示数组.append({"文本": "[center][font_size=%d]%s[/font_size][/center]" % [默认字体+10, 资源.item_name]})
		提示数组.append({"文本":tr("<阶级>")%阶级})
		提示数组.append({"模式":"分栏","间距":0,"分栏组": [资源.数值,核心文本]})
	if 简化:
		pass
	elif 资源.宝石词条.is_empty():
		提示数组.append({"文本":tr("<宝石词条开发中>")})
	else :
		提示数组.append({"模式":"分隔线","边距":10,"宽度":3})
		提示数组.append({"文本": "[font_size=%d]%s[/font_size]" % [默认字体+2,tr("【随机词条】")]})
		# 遍历宝石资源中的词条配置##以下内容未翻译,开发时再翻译
		for 词条名 in 资源.宝石词条:
			if 资源.随机词条.has(词条名):
				# 所有数据从传入的宝石资源中获取
...
func 解析实体(实体:游历实体,_参数:Dictionary={}):
	...
	提示数组.append({"文本":tr("<攻击力>")%实体.攻击力})
	提示数组.append({"文本":tr("<暴击力>")%实体.暴击力})
	提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
	提示数组.append({"文本":tr("<手持武器>")%实体.武器名称})
func 通用提示(价格:int=0,简介:String="",参数:Dictionary={"样式":{"移动":["鼠标右键"],"选中":["鼠标左键"]}}):
	if 简介.is_empty():
		if 价格>=1:
			提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
			提示数组.append({"文本":tr("<基础价值>")%[数值色,价格]})
	else :
		if 价格>=1:
			提示数组.append({"文本":tr("<基础价值>")%[数值色,价格]})
		提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
		提示数组.append({"宽度":500,"文本":tr("<物品简介>")%[简介]})
	if 参数.has("样式"):
		# 1. 初始化分栏组，固定以“操作说明:”开头
		var 分栏组: Array = [tr("<操作说明>")]
...
func 生成蓝图(配方名称:String):
	...
	提示数组.append({"文本":"[center][font_size=%d]%s %s[/font_size][/center]"%[
		默认字体+10,计划.表格.翻译名称(配方名称),缓存升星字符]})
	if 配方等级>0:
	...
	提示数组.append({"文本":tr("<自动最低时间>")%[数值色,制作耗时]})
	if 残缺图纸数量>=1:
		var 研究力:=计划.装备.研究力
		var 阶级:int=计划.表格.蓝图数据(配方名称,"阶级")
		var 蓝图纸点数:int=int(阶级*(2+0.5*研究力))
		提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
		提示数组.append({"文本":tr("<残缺图纸次数>")%[数值色,残缺图纸数量]})
		提示数组.append({"文本":tr("<蓝图点数获得>")%[数值色,蓝图纸点数]})
		提示数组.append({"文本":tr("<点数来源说明>")%[阶级*2,int(阶级*0.5*研究力)]})
	# 构建内容字符串
	var 分栏组:Array=[]
	var 精华:bool=false
	...
	if 分栏组.size()>=1:
		提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
		提示数组.append({"文本": "[font_size=%d]%s[/font_size]" % [默认字体+2,tr("【制作材料】")]})
		提示数组.append({"模式":"分栏","间距":10,"分栏组":分栏组})
	if 精华:
		提示数组.append({"文本":tr("<制作精华占用>")})
func 队列消息(制作参数:Array,队列序号:int):
	...
	if 队列序号+1>制作参数.size():
		...
		提示数组.append({"模式":"分栏","分栏组":[冷却/制作时长,tr("<制作时间>")%[数值色,制作时长,冷却]]})
		提示数组.append({"文本":tr("<提速说明>")})
		var 消耗精华:int=int(计划.表格.蓝图数据(制作物品名,"精华"))
		if 消耗精华>=1:
			提示数组.append({"文本":tr("<制作中精华占用>")%[数值色,消耗精华]})
