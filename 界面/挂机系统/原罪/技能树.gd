...
func _ready() -> void:
	...
	var 研究制作队列:Array = 计划.队列技能()
	刷新研究队列(研究制作队列)
	if 研究制作队列.size()>=1:
		动作进度条.开始动作("技能研究",5,self)
		清空队列.visible=true
	else :清空队列.visible=false
	加入队列.pressed.connect(加入制作队列)
	清空队列.pressed.connect(清空制作队列)
	研究倍率.value=float(计划.技能队列参数("研究强度"))
...
func 更新制作强度():
	计划.技能队列参数("研究强度",int(研究倍率.value))
	研究经费.text="每5秒消耗%.0f金币\n获取%d的研究进度"%[5*int(研究倍率.value**2),5*研究倍率.value]
func 加入制作队列():
	var 前置技能=计划.数据技能树(技能名称, "前置技能")
	for 前置技能名称 in 前置技能:
		var 前置技能当前等级 = 计划.数据技能树(前置技能名称, "等级")
		if 前置技能当前等级 < 1:
			计划.语法糖通知("需要先至少获得一级%s技能"%前置技能名称,"技能树")
			return
	var 最大等级=计划.数据技能树(技能名称, "最大等级")
	var 当前等级=计划.数据技能树(技能名称, "等级")
	if 当前等级>=最大等级:
		计划.语法糖通知("技能已满级","技能树")
		return
	var 更新制作队列:Array = 计划.队列技能(技能名称,1)
	if not 动作进度条.执行动作中:
		动作进度条.开始动作("技能研究",5,self)
		清空队列.visible=true
	刷新研究队列(更新制作队列)
...
func 刷新研究队列(研究制作队列=计划.队列技能()):
	...
	if 研究队列节点.size()==研究制作队列.size():
		for i in range(研究队列节点.size()):
			...
			var 研究进度 = 计划.数据技能树(技能的名称, "研究进度")
			var 研究费用 = 计划.数据技能树(技能的名称, "研究费用")
...
func 加载技能文本(技能名=技能名称):
	...
	if not 技能字典 is Dictionary:
		...
		var 所属系统 = 技能字典["系统"]
		var 当前等级 = 计划.数据技能树(技能名称, "等级")
		var 研究进度 = 计划.数据技能树(技能名称, "研究进度")
		var 研究费用 = 计划.数据技能树(技能名称, "研究费用")
		var 技能强度 = 计划.数据技能树(技能名称, "强度")
		...
		for 单个前置技能名称 in 前置技能数组:
			var 前置技能当前等级 = 计划.数据技能树(单个前置技能名称, "等级")
			if 前置技能当前等级 < 1:
...
func 更新细线():
	...
	for 节点 in 范围节点.get_children():
		if 节点 is 技能节点:
			var 当前技能名称=节点.技能文本
			var 前置技能:Array=计划.数据技能树(当前技能名称, "前置技能")
			缓存字典[当前技能名称]={}
			缓存字典[当前技能名称]["节点"]=节点
