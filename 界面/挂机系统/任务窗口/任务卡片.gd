...
func _ready() -> void:
	...
	计划.表格.更新_翻译.connect(完整更新流程)
func 完整更新流程():
	任务描述数组=[]
	for 文本:String in 任务数据.任务描述:
		任务描述数组.append(tr(文本))
	更新标题()
	加载任务说明()
func 更新标题():
	...
	任务名称.text=tr(任务数据.任务类型)+str(任务序号)+"."+tr(任务数据.任务名称)+后缀
func 加载任务说明(更新任务:Array[任务资源]=[]):
	...
	if not 前置:
		...
		if 前置:
			...
			任务描述.text = tr("<任务未解锁>")%[任务完成量,任务数量]+"\r"+tr("<前置任务>")%tr(未完成前置任务[0])
	if 前置:#前置任务条件满足
		解析文本()
		加载参数()
func 加载参数():
...
func 解析文本():
	...
	if not 任务文本.find("{动态翻译}")==-1 and not 任务数据.动态翻译=="":
		任务文本=任务文本.replace("{动态翻译}",tr(任务数据.动态翻译)%任务数据.目标值)
		...
		if not 任务数据.任务完成 or ["对话","刷新"].has(功能数组[0]):
			...
			if 功能数组[0]=="对话":
				...
				按钮.pressed.connect(func():
					if 任务数据.任务完成:
						计划.语法糖通知(tr("任务奖励已领取"),"任务提示")
						return
					完成任务())
			elif 功能数组[0]=="提交":
				按钮.pressed.connect(func():
					if 任务数据.任务完成:
						计划.语法糖通知(tr("任务奖励已完成,无需重复"),"任务提示")
						return
...
func 更新按钮文本():
	if 任务领取按钮:
		if 任务数据.任务状态:任务领取按钮.text="挂起"
		else :任务领取按钮.text="接受任务"
	if 刷新按钮:
		if 任务数据.任务状态:刷新按钮.text="前往"
		else :刷新按钮.text="刷新"
