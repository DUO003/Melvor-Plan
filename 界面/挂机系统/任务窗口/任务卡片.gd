...
func 初始化任务():
	if not 数据:
		return
	任务详情=计划.任务.任务字典[数据.任务名称]
	$"红点提示".红点条目="任务_"+数据.任务名称
	var 未完成前置任务:Array=计划.任务.统计未完成前置任务(数据.任务名称)
	var 任务数量:int=任务详情.get("前置任务",[]).size()
	var 任务完成量:int=任务数量-未完成前置任务.size()
	print(数据.任务名称,"前置数量%d/%d"%[任务完成量,任务数量])
	...
	$"红点提示".点击逻辑=func():
		if not 计划.红点.红点数据.has("任务_"+数据.任务名称):
			计划.红点.红点数据["任务_"+数据.任务名称]=1
			计划.更新红点.emit("任务_"+数据.任务名称)
			计划.语法糖通知("本次登录期间<%s>任务的红点提示已隐藏"%数据.任务名称)
	更新标题()
	解析功能按钮()
func 更新标题():
	var 后缀=""
	if 数据.任务完成:
		后缀="(已完成)"
	elif 计划.红点.获取红点状态("任务_"+数据.任务名称)==1:
		if 数据.任务状态:
			后缀="(进行中)"
		else :
			后缀="(可领取)"
		#print(任务名称,任务领取状态.get(任务名称,false))
	$"任务名称".text=数据.任务类型+str(任务序号)+"."+数据.任务名称+后缀
	
func 加载任务说明(任务数组=null):
	if 任务数组 is Array:
		if 数据.任务名称 in 任务数组:%"任务描述".text=解析文本()
...
func 解析文本():
	...
	if not 任务文本.find("{当前进度}")==-1 and "进度描述" in 任务详情:
		var 缓存任务:Dictionary=计划.任务.任务全局
		if 数据.任务名称 in 缓存任务:
			var 替换文本=任务详情["进度描述"]
			var 任务当前=缓存任务[数据.任务名称]["当前进度列表"]
			var i=0
			for 条件 in 任务详情.get("前置条件",[]):
				var 条件文本="{"+条件["条件名称"]+"}"
				var 任务进度值=任务当前[i]
				if 任务进度值 is float:
					任务进度值="%.0f"%任务进度值
...
func 解析功能按钮():
	...
	for 功能数组 in 任务详情.get("功能按钮"):
		if not 数据.任务完成 or 功能数组[0]=="对话":
			var 按钮=Button.new()#目前仅存在按钮一种功能后续可能会追加其他选项
			按钮.text=功能数组[1]
			if 功能数组[0]=="对话":
				按钮.pressed.connect(func():启动对话(数据.任务名称))
			elif 功能数组[0]=="解锁":
	...
	elif 计划.任务.任务全局.has(数据.任务名称):
		var 目标值=计划.任务.任务全局[数据.任务名称]["当前进度列表"].size()
		var 进度值=计划.任务.任务全局[数据.任务名称]["完成总进度"]
		if 目标值>进度值:
			隐藏或显示节点(按钮数组,false)
			var 进度条=梅任务进度条.new()
			进度条.任务名称=数据.任务名称
