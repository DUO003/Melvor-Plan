...
func 解析文本():
	...
	if not 任务文本.find("{任务}")==-1 and 任务数据.循环任务>=1:
		任务文本=任务文本.replace("{任务}","%d/%d"%[任务数据.打包数据.已完成循环任务,任务数据.循环任务])
	...
	for 功能数组:Array in 任务数据.功能按钮:
		if not 任务数据.任务完成 or ["对话","刷新"].has(功能数组[0]):
			...
			if 功能数组[0]=="对话":
				...
				if 功能数组.size()>=3:
					需求体力=功能数组[2]
				按钮.pressed.connect(任务数据.刷新任务.bind(需求体力))
			elif 功能数组[0]=="解锁":
				...
				按钮.pressed.connect(func():
					if 任务数据.任务完成:
						...
						return
					var 打包数据:任务打包资源=任务数据.打包数据
					if 打包数据.已完成循环任务>=任务数据.循环任务:
						打包数据.已完成循环任务-=任务数据.循环任务
						完成任务()
