...
func 初始化任务():
	...
	$"红点提示".点击逻辑=func():
		if not 计划.红点.红点数据.has("任务_"+任务名称):
			...
			计划.语法糖通知("本次登录期间<%s>任务的红点提示已隐藏"%任务名称)
	更新标题()
	解析功能按钮()
func 更新标题():
	var 后缀=""
...
func 解析功能按钮():
	...
	if 任务进度==1:
		...
		var 目标值=计划.任务.任务全局[任务名称]["当前进度列表"].size()
		var 进度值=计划.任务.任务全局[任务名称]["完成总进度"]
		if 目标值>进度值:
			隐藏或显示节点(按钮数组,false)
			var 进度条=梅任务进度条.new()
			进度条.任务名称=任务名称
...
func 接受任务():
	if not 任务进度==1:
		任务领取状态[任务名称]=not 任务领取状态.get(任务名称,false)
		更新标题()
		加载任务说明()
		更新按钮文本()
		计划.更新红点.emit("任务_"+任务名称)
