...
#不同的地图数据
@export var 地图资源字典: Dictionary[String,Dictionary]
#保存TileMapPattern数据
@export var 地图资源: TileMapPattern
var 当前地图名:String="起始之地"
@export var 建筑数据:Dictionary[Vector2i,建筑资源]={}
# 保存原始单元格的起点坐标（绝对坐标）- 改为导出变量类型，满足你的要求
@export var 图案起点坐标: Vector2i = Vector2i(0, 0)
@onready var 玩家: 大地图玩家 = $"../玩家"
...
func 保存地图数据() -> void:
	...
	地图存档[当前地图名]={"坐标":图案起点坐标,"图块":地图资源,"玩家":玩家.global_position,"建筑":建筑数据}
	#print("保存成功地图",地图存档[当前地图名])
func 切换地图() -> void:
	if not 地图资源字典.has(当前地图名) or not 地图资源字典[当前地图名].has_all(["图块","坐标","玩家"]):
		print("缺少默认值")
		breakpoint
		return
	地图层.clear()
	...
	鼠标层.clear()
	碰撞层.clear()
	var 地图的数据:Dictionary=地图资源字典[当前地图名]
	var 数据字典:Dictionary=地图存档.get(当前地图名,{})
	加载地图数据(地图层,地图的数据)
	if 数据字典.is_empty() and 地图的数据.has_all(["家具","家具坐标"]):
		数据字典["坐标"]=地图的数据.家具坐标
		数据字典["图块"]=地图的数据.家具
		加载地图数据(家具层,数据字典)
	else :
	...
	if 数据字典.has("玩家"):
		...
		玩家.position=地图的数据.玩家
	玩家.出生点=地图的数据.玩家
	if 数据字典.has("建筑"):
		建筑数据=数据字典.建筑
	elif 地图的数据.has("建筑"):
		var 转码建筑数据:Dictionary[Vector2i,建筑资源]={}
		for 坐标 in 地图的数据.建筑:
			转码建筑数据[坐标]=地图的数据.建筑[坐标]
		建筑数据=转码建筑数据
	else :
		建筑数据={}
	print("地图成功加载")
## 加载地图数据从TileMapPattern
func 加载地图数据(地图: TileMapLayer,数据字典:Dictionary) -> void:
	if 数据字典.is_empty():
		return
	var 资源: TileMapPattern=数据字典.图块
	var 坐标:Vector2i=数据字典.坐标
	if 地图 and 资源 and not 资源.is_empty():
		地图.set_pattern(坐标, 资源)# 用保存的导出变量起点坐标还原，避免位置偏移
...
func 放置方块(方块坐标:Vector2i,物品:物品方块):
	...
	var 建筑文件:=建筑资源.new(物品.item_name)
	if 物品.瓦片功能=="解锁窗口":
		计划.解锁窗口(物品.功能参数)
	elif 物品.瓦片功能=="显示提示":
		建筑文件.文本数据="点击方块打开设置提示"
	建筑数据[方块坐标]=建筑文件
...
func 加载方块功能(单元格坐标,列,排,方块名称,瓦片功能):
	...
	if not 建筑数据.has(单元格坐标) or not 建筑数据[单元格坐标] is 建筑资源:
		建筑数据[单元格坐标]=建筑资源.new(方块名称)
	var 建筑文件:建筑资源=建筑数据[单元格坐标]
	if 瓦片功能=="":
		pass
	elif ["解锁窗口","图书馆"].has(瓦片功能):
		var 按钮:=Button.new()
		按钮.text=方块名称
...
func 移除方块(方块坐标:Vector2i):
	if 建筑数据.has(方块坐标) and not 建筑数据[方块坐标].储物空间.is_empty() or 建筑数据[方块坐标].产物:
		计划.语法糖通知("你不能移除有物品的建筑")
		return
	var 方块名称:=获取方块名称(方块坐标)
	if not 方块名称=="":
