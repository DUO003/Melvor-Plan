...
@export var 颜色字典: Dictionary[int,Color] = {
	-1:Color(0.702, 0.702, 0.702),
	0:Color(0.8, 0.902, 1.0, 0.4)}
# 绘制粗细
@export var 粗细: int = 5:
	set(值):
		粗细=值
...
var 打印日志:Array=[]
func 打印(显示文本:String,中心点:Vector2,顶点列表:PackedVector2Array,解锁:int):
	...
	for 打印数据:Dictionary in 打印日志:
		if 打印数据.has_all(["显示文本","中心点","顶点列表","解锁"]):
			...
			if not 打印数据.解锁==-1:
				...
				var 位置:Vector2=中心点+Vector2(文本宽度*-0.5,半径-字体大小)
				draw_string_outline(字体,位置,显示文本,HORIZONTAL_ALIGNMENT_RIGHT,-1,字体大小,10,
				Color(0.8, 0.903, 1.0, 0.549))
				draw_string(字体,位置,显示文本,HorizontalAlignment.HORIZONTAL_ALIGNMENT_LEFT,-1,字体大小,文字颜色)
			if 颜色字典.has(打印数据.解锁):
				var 填充颜色数组: PackedColorArray = PackedColorArray()
				for i in 顶点列表:  # 遍历顶点列表，为每个顶点添加相同的填充颜色
					填充颜色数组.append(颜色字典[打印数据.解锁])
