...
func 领取奖励():
	...
	if 队列完成>=药水长度:
		...
		for i in 20:
			统计奖励.append(领取单次(i,完成数量,药水长度))
		炼金数量+=药水长度*完成数量
		队列完成-=药水长度*完成数量
	if 队列完成>=1:
		for i in 队列完成:
			统计奖励.append(领取单次(炼金数量,1,药水长度))
			炼金数量+=1
		队列完成=0
	...
	计划.任务.任务通用(计划.任务.进度类型.炼金,任务推进次数,催化剂,{"催化剂":催化剂})
	计划.更新_UI.emit()
	计划.保存存档("领取炼金奖励")
	计划.steam.解锁成就("初试炼金")
func 扣除材料(数量)->bool:
...
func 扣除炼金材料(数量:int=1):
	...
	if 数量 > 0:
		...
		计划.语法糖获得物品(催化剂名称, -数量)
##如果药水数量为0替换显示为贤者点数
func 领取单次(炼金序号:int,完成数量:int,药水长度:int=药水序列.size()):
	var 数量=查询药水(炼金序号,"数量",药水长度)*完成数量
	var 名称=查询药水(炼金序号,"名称",药水长度)
...
func 配方格式检查(生成通知:bool=false)->bool:
	...
	if not (数组长度匹配 and 材料数量达标):# 3. 检查材料数组长度匹配且最少3种材料
		if 生成通知:
			if not 材料数量达标:计划.语法糖通知("添加失败:至少放入3种炼金材料","炼金窗口")
			else:计划.语法糖通知("添加失败:配方损坏","炼金窗口")
		return false # 配方错误
	var 已记录的材料名:Array=[]
...
func 更新药水数量(属性总和字典 = 计算配方点数()):
	...
	for 选中药水 in 药水序列:
		...
		if 药水数量配置.size() >= 2: # 配置有效：[阈值,最大数量]
			...
			最终数量 = 1
			药水成功.append(true)
		炼金点数.append(max(5,属性总值**0.75*min(1,0.2+运气)))
		药水数量.append(最终数量)
func 计算药水权重(属性总和字典):
...
func 移动配方(后移: bool) -> bool:
	...
	var 配方数组:Array = 计划.手工.读取炼金配方列表()
	var 目标索引 = 配方数组.find(self)
	if 后移:# 后移：交换当前索引和下一个索引的元素
		var 临时变量 = 配方数组[目标索引]
		配方数组[目标索引] = 配方数组[目标索引 + 1]
		配方数组[目标索引 + 1] = 临时变量
	else:# 前移：交换当前索引和上一个索引的元素
		var 临时变量 = 配方数组[目标索引]
		配方数组[目标索引] = 配方数组[目标索引 - 1]
		配方数组[目标索引 - 1] = 临时变量
	计划.更新_UI.emit()
