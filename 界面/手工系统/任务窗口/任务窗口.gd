...
func _ready() -> void:
	...
	super._ready()
	任务字典=计划.任务.任务字典
	var 显示任务数量=计划.窗口状态管理("任务","显示任务数量",1)#需要先获取值
	%"显示任务".selected=显示任务数量 if 显示任务数量 is int and 显示任务数量>=0 and 显示任务数量<=2 else 1
	var 以完成任务=计划.窗口状态管理("任务","以完成任务",true)
	%"以完成任务".button_pressed = 以完成任务 if 以完成任务 is bool else true
	await 初始化所有任务容器()
	任务栏初始化()
	%"以完成任务".pressed.connect(func():
		计划.窗口状态管理("任务","以完成任务",null,%"以完成任务".button_pressed)
		刷新任务显示())
	%"显示任务".item_selected.connect(func(序号):
		计划.窗口状态管理("任务","显示任务数量",null,序号)
		刷新任务宽度())
	%"启用全屏".button_pressed=计划.配置文件.get("全屏",false)
	%"启用全屏".toggled.connect(func(条件):计划.切换全屏(条件))
	%"标签".tab_clicked.connect(func(标签序号):计划.窗口状态管理("任务","标签",null,标签序号))
	%"支线任务".tab_clicked.connect(func(标签序号):计划.窗口状态管理("任务","支线任务",null,标签序号))
	if 计划.跳转设置:
		print("跳转设置成功")
		切换到设置()
...
func 加载窗口状态(选项卡:TabContainer,名称:String):
	var 标签=计划.窗口状态管理("任务",名称,0)#这里的0是假设存档没有数据返回值
...
func 初始化所有任务容器():
	for 容器 in 配置:
		清除子节点(容器)
	任务文本字典={}
	var 任务的卡片:任务卡片=preload("res://界面/手工系统/任务窗口/任务卡片.tscn").instantiate()
	加载任务完成统计()
	加载窗口状态(%"标签","标签")
	加载窗口状态(%"支线任务","支线任务")
	var 显示任务数量=%"显示任务".selected+1
