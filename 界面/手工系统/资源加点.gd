...
func _ready() -> void:
	if not Engine.is_editor_hint():
		...
		类型="基础"
		回复速度.text="%s回复+%.2f"%[资源名称,增加回复速度]
func 更新文本():
	if not Engine.is_editor_hint():
		更新回复速度()
	if 回复速度 and 点数:
		if Engine.is_editor_hint():
			回复速度.text="%s回复+%.2f"%[资源名称+"质",增加回复速度]
		else :
			var 制作力:float=计划.装备.制作力
			var 上限:int
			if 类型=="特殊":
				上限=int(pow(制作力, 1.0 / 1.5))
				回复速度.text="%s自动制作+%.0f\r上限:%d,费用:2"%[计划.手工.返回资源信息(资源名称,"显示名"),增加回复速度,上限]
			else :
				上限=int(计划.装备.制作力)+5
				回复速度.text="%s回复+%.2f\r上限:%d,费用:1"%[计划.手工.返回资源信息(资源名称,"显示名"),增加回复速度,上限]
		点数.text="%d"%资源点数
func 更新回复速度():
...
func 尝试加点(值:int):
	if 计划.手工.基础资源加点(资源名称,值):
		更新回复速度()
		if 值>0:计划.语法糖通知("加点成功,当前%s点数:%d"%[资源名称,资源点数],"资源加点")
		else :计划.语法糖通知("代币退还,点数:%d"%[资源点数],"资源加点")
		计划.手工.重算资源回复()
		计划.更新_UI.emit()
	else :计划.语法糖通知("操作失败","资源加点")
