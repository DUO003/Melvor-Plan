...
@export var 水平排列容器:HBoxContainer
...
func _ready() -> void:
	...
	if 水平排列容器:
		水平排列容器.add_theme_constant_override("separation",-10)
...
func set_text(txt : String) -> void:
	_text = txt
	if txt.ends_with("(*)"):
		button_main.text = txt.trim_suffix("(*)").trim_suffix(".gd")
		changes.text="未保存"
		changes.visible=true
		return
	button_main.text = txt.trim_suffix(".gd")
