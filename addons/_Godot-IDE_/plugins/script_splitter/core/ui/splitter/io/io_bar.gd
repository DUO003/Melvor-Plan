...
func _setup() -> void:
	if !_root:
		return
	if enable_expand:
		make_function(&"EXPAND", FILL_EXPAND, _tr("让分屏标签容器全屏(未分屏不生效)"))
		#Expand/unexpand Current Tab Container
	if enable_horizontal_split:
		make_function(&"SPLIT_COLUMN", SPLIT_CPLUS_TOOL, _tr("增加左右分屏"))
		#Split to new column
		make_function(&"MERGE_COLUMN", SPLIT_CMINUS_TOOL, _tr("移除左右分屏"))
		#Merge current column
	if enable_vertical_split:
		make_function(&"SPLIT_ROW", SPLIT_RPLUS_TOOL, _tr("增加上下分屏"))
		#Split to new row
		make_function(&"MERGE_ROW", SPLIT_RMINUS_TOOL, _tr("移除上下分屏"))
		#Merge current row
	if enable_sub_split:
		make_function(&"SPLIT_SUB", SPLIT_PLUS_TOOL, _tr("增加文件分页"))
		#Sub Split current editor
		make_function(&"MERGE_SPLIT_SUB", SPLIT_MINUS_TOOL, _tr("移除文件分页"))
		#Merge sub split of current editor
	if enable_pop_script:
		make_function(&"MAKE_FLOATING", ATOP, _tr("独立窗口打开"))
		#Make separate window
	if enable_templates:
		make_function(&"_T_EDITOR", TEMPLATE_EDITOR, _tr("添加收藏"))
		#Make new split template
