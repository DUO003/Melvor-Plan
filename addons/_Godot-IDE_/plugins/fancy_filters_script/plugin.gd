...
func _enter_tree() -> void:
	...
	if container:				
		...
		if is_instance_valid(menu):
			...
			var msg : String = _get_traduce("脚本过滤器开关")
			#_get_traduce("Show/Hide Scripts and Filters Panel")
			...
			msg = _get_traduce("切换脚本过滤器")
			#Toggle Script Info/Script List Panel (Only if the separated option enabled)
			_add_input(pop, msg, _c_input_switch_panels)
			_id_switch_panels = total
				
			total = pop.item_count
			msg = _get_traduce("移动脚本过滤器")
			#Toggle Position Script and Filters Panel
