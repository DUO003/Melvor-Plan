...
## 向背包添加物品返回添加成功
func add_item(背包: String, 物品: ItemData) -> bool:
	var 同名数组 = find_item_data_by_item_name(背包, 物品.item_name)#通过物品名称找所有同名物品
	...
	if 物品 is StackableData:# 处理可堆叠物品的逻辑分支
		...
		var 背包数据:ContainerData=_container_repository.get_container(背包)
		for 同名物品 in 同名数组:# 遍历已存在的同类型物品，尝试堆叠
			if 同名物品 is StackableData:
				if not 同名物品.满堆叠():# 仅处理未堆满的物品槽位
					...
					GBIS.sig_inv_item_updated.emit(背包, 物品格子[0])#发射物品更新信号
					if 物品.数量 <= 0:
						return true#堆叠无剩余返回成功
	var 可用格 = _container_repository.get_container(背包).add_item(物品)
	if not 可用格.is_empty():# 若成功找到可放置的格子并添加物品
		GBIS.sig_inv_item_added.emit(背包, 物品, 可用格)# 发射物品新增信号
		return true
	return false# 无可用格子/堆叠失败，返回添加失败
## 尝试把正在移动的物品堆叠到这个格子上
func stack_moving_item(背包: String, grid_id: Vector2i) -> void:
	if not GBIS.moving_item_service.moving_item:
		return
	var item_data = find_item_data_by_grid(背包, grid_id)
	...
	if item_data.item_name == GBIS.moving_item_service.moving_item.item_name:
		...
		GBIS.sig_inv_item_updated.emit(背包, grid_id)

## 尝试放置正在移动的物品到这个格子
func place_moving_item(背包: String, grid_id: Vector2i) -> bool:
	if place_to(背包, GBIS.moving_item_service.moving_item, grid_id):
		GBIS.moving_item_service.clear_moving_item()
		计划.保存存档("背包:物品格子移动")
...
## 使用物品（默认：鼠标右键点击格子）
func use_item(背包: String, grid_id: Vector2i) -> bool:
	var item_data = find_item_data_by_grid(背包, grid_id)
	if not item_data:
		return false
	if item_data is 标准物品 or item_data is 物品宝石 or item_data is 物品装备:
		计划.更新_背包物品信息.emit(item_data,背包)
		...
		if GBIS.equipment_slot_service.try_equip(item_data):
			remove_item_by_data(背包, item_data)
			return true
...
## 分割物品
func split_item(背包: String, grid_id: Vector2i, offset: Vector2i, base_size: int,背包本体=null) -> ItemData:
	var inv = _container_repository.get_container(背包)
	if inv:
...
## 快速移动（默认：Shift + 鼠标右键）
func quick_move(背包: String, grid_id: Vector2i) -> void:
	var target_inventories = _container_repository.get_quick_move_relations(背包)
	var item_to_move = _container_repository.get_container(背包).find_item_data_by_grid(grid_id)
	...
	for target_container in target_inventories:
		...
		if add_item(target_container, item_to_move):
			remove_item_by_data(背包, item_to_move)
			break
		elif item_to_move is StackableData:
			GBIS.sig_inv_item_updated.emit(背包, grid_id)

## 增加背包间的快速移动关系
func add_quick_move_relation(背包: String, target_inv_name: String) -> void:
	_container_repository.add_quick_move_relation(背包, target_inv_name)

## 删除背包间的快速移动关系
func remove_quick_move_relation(背包: String, target_inv_name: String) -> void:
	_container_repository.remove_quick_move_relation(背包, target_inv_name)

## 删除背包中的物品，成功后触发 sig_inv_item_removed
func remove_item_by_data(背包: String, item_data: ItemData) -> void:
	if _container_repository.get_container(背包).remove_item(item_data):
		GBIS.sig_inv_item_removed.emit(背包, item_data)
