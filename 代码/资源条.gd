...
func 提示文本()->梅提示数据:
	...
	var 数值色:String=提示数据.数值色
	提示数据.提示数组=[{"文本":"[center][font_size=%d]%s[/font_size][/center]"%[
		提示数据.默认字体+10,计划.手工.返回资源信息(资源名称,"显示名")]}]
	提示数据.提示数组.append({"文本":"资源类型:%s"%[类型]})
	提示数据.提示数组.append({"文本":"资源总数:[color=%s]%d[/color]"%[数值色,int(当前数量+背包内数量)]})
	if not 类型=="特殊":
		提示数据.提示数组.append({"文本":"资源条[color=%s]%.0f/%s[/color]"%[数值色,当前数量,上限文本]})
	提示数据.提示数组.append({"文本":"背包:[color=%s]%d[/color]"%[数值色,背包内数量]})
	提示数据.提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
	var 简介:String=计划.手工.返回资源信息(资源名称,"简介")
	if 类型=="特殊":
		提示数据.提示数组.append({"文本":"自动制作[color=%s]%s/%.0f[/color]"%[数值色,资源回复文本,计划.手工.精华数量()]})
		提示数据.提示数组.append({"文本": "[font_size=%d]【资源购买】[/font_size]" %[提示数据.默认字体+2]})
		提示数据.提示数组.append({"文本":"消耗50金币点击购买[color=%s]1[/color]%s"%[数值色,资源名称]})
	else :
		提示数据.提示数组.append({"文本": "[font_size=%d]【资源回复】[/font_size]" %[提示数据.默认字体+2]})
		if 类型=="高级":
			提示数据.提示数组.append({"文本":"点击+[color=%s]%.1f[/color]/每次"%[数值色,基础量]})
		else :
			提示数据.提示数组.append({"文本":"基准+[color=%s]%d[/color]/分钟"%[数值色,计划.手工.读取基础资源回复(资源名称)]})
			提示数据.提示数组.append({"文本":"当前+[color=%s]%s[/color]/分钟"%[数值色,资源回复文本]})
			提示数据.提示数组.append({"文本":"点击+[color=%s]%d[/color]/每次"%[数值色,基础量]})
	提示数据.提示数组.append({"文本":"长按时每秒[color=%s]2[/color]次"%[数值色]})
	提示数据.通用提示(0,简介,{"样式":{"点击回复":["鼠标左键"]}})
	return 提示数据
