...
func 提示文本()->梅提示数据:
	...
	提示数据.提示数组=[{"文本":"[center][font_size=%d]%s[/font_size][/center]"%[
		提示数据.默认字体+10,tr(计划.手工.返回资源信息(资源名称,"显示名"))]}]
	提示数据.提示数组.append({"文本":tr("<资源类型>")%[tr(类型)]})
	提示数据.提示数组.append({"文本":tr("<资源总数>")%[数值色,int(当前数量+背包内数量)]})
	if not 类型=="特殊":
		提示数据.提示数组.append({"文本":tr("<资源条>")%[数值色,当前数量,上限文本]})
	提示数据.提示数组.append({"文本":tr("<背包>")%[数值色,背包内数量]})
	提示数据.提示数组.append({"模式":"分隔线","边距":10,"宽度":5})
	var 简介:String=tr("<%s_简介>"%资源名称)
	if 类型=="特殊":
		提示数据.提示数组.append({"文本":tr("<自动制作>")%[数值色,资源回复文本,计划.手工.精华数量()]})
		提示数据.提示数组.append({"文本": "[font_size=%d]%s[/font_size]" %[提示数据.默认字体+2, tr("<资源购买标题>")]})
		提示数据.提示数组.append({"文本":tr("<消耗购买>")%[数值色,tr(资源名称)]})
	else :
		提示数据.提示数组.append({"文本": "[font_size=%d]%s[/font_size]" %[提示数据.默认字体+2, tr("<资源回复标题>")]})
		if 类型=="高级":
			提示数据.提示数组.append({"文本":tr("<点击每次增加>")%[数值色,基础量]})
		else :
			提示数据.提示数组.append({"文本":tr("<基准每分钟增加>")%[数值色,计划.手工.读取基础资源回复(资源名称)]})
			提示数据.提示数组.append({"文本":tr("<当前每分钟增加>")%[数值色,资源回复文本]})
			提示数据.提示数组.append({"文本":tr("<点击每次增加>")%[数值色,基础量]})
	提示数据.提示数组.append({"文本":tr("<长按时每秒>")%[数值色]})
	提示数据.通用提示(0,简介,{"样式":{tr("<点击回复>"):["鼠标左键"]}})
	return 提示数据
