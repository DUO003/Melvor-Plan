...
func _更新_UI():
	...
	if Engine.is_editor_hint():
		...
		熟练数值.text="[img=40x40]%s[/img]%s%s:%d/%d"%[计划.表格.道具贴图("熟练").resource_path,tr("熟练"),
			增益文本,熟练,熟练上限]
		精通数值.text="[img=40x40]%s[/img]%s:%d/%d"%[计划.表格.道具贴图("精通").resource_path,tr("精通"),
			精通,精通上限]
		if 熟练上限==-1:
			熟练=0
			熟练上限=1
