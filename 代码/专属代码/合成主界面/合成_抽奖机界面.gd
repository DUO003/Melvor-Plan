...
func 抽奖机逻辑分支():
	...
	if 抽奖机修复.has("修复成功"):
		...
		%提交.pressed.connect(func():
			if 抽奖机修复.has("金币"):
				引擎.屏幕.滚动提示("已付费","手工提示")
			elif 初始化.梅存档["金币"]>=2000:
				初始化.梅存档["金币"]-=2000
				抽奖机修复+=["金币"]
				初始化.梅存档["手工"]["抽奖机修复"]=抽奖机修复
				解锁判定()
				初始化.保存存档("解锁窗口:抽奖机")
				if 抽奖机修复.has("修复成功"):
					初始化.切换场景("合成_抽奖机界面","合成界面",true)
			else :
				引擎.屏幕.滚动提示("金币不足","手工提示"))
func 加载研究方向():
	研究方向=初始化.梅存档["手工"].get("研究方向",[])
	if not 研究方向.has("符文"):
		研究方向+=["符文"]
		初始化.梅存档["手工"]["研究方向"]=研究方向
	研究序号=初始化.梅存档["手工"].get("研究序号",-1)
	研究节点.selected=研究序号
	研究节点.clear()
	for 研究选项 in 研究方向:
		研究节点.add_item(研究选项)
...
func 解锁判定():
	抽奖机修复=初始化.梅存档["手工"].get("抽奖机修复",[])
	if not 抽奖机修复.has("修复成功")and 抽奖机修复.has("金币")and 初始化.梅存档["手工"].get("等级",-1)>=1:
		抽奖机修复+=["修复成功"]
		初始化.梅存档["手工"]["抽奖机修复"]=抽奖机修复
	%"解锁提示".text="抽奖机状态:%s
修复条件
...
2.金币2000%s"%[
	...
	"(完成✔)"if 初始化.梅存档["手工"].get("等级",-1)>=1 else"✘",
...
当前定向选项 "+定向选项+"
当前研究费用："+str(研究费用)
	var 蓝图池=初始化.梅手工单例.筛选符合条件蓝图([],[研究节点.text],[],[])
	%"研究配方贴图".texture=load(梅表格.装备蓝图[蓝图池[randi()%蓝图池.size()]][icon])
	#测试抽奖([0.7, 0.21, 0.08])
...
func 研究():
	...
	if 初始化.检查背包物品数量("蓝图纸")>=研究费用:
		...
		if 对照数组[奖品]=="残缺蓝图" or 对照数组[奖品]=="装备蓝图":
			蓝图池=初始化.梅手工单例.筛选符合条件蓝图([],[研究节点.text],[],阶级条件)
		elif 对照数组[奖品]=="特殊蓝图":
			蓝图池=初始化.梅手工单例.筛选符合条件蓝图([],[],[],阶级条件,["特殊"])
		else :
			蓝图包.shuffle()
			蓝图包=蓝图包[0]
			蓝图池=初始化.梅手工单例.筛选符合条件蓝图([],[],[],阶级条件,[蓝图包])
		#print(对照数组[奖品],":",蓝图池)
		#print("阶级条件:",阶级条件)
		蓝图池.shuffle()
		...
		if 对照数组[奖品]=="残缺蓝图":
			...
			%蓝图属性.text=初始化.预生成文本(蓝图池[0],false,[40,30,100])
		elif 对照数组[奖品]=="特殊蓝图":
		...
		%动画.play("研究")
		初始化.梅任务单例.完成任务研究(对照数组[奖品],研究费用缓存,1)
	else :
...
func _蓝图下一页() -> void:
	if 动画显示页码<动画包显示支持数组.size()-1:
		动画显示页码+=1
	%蓝图属性.text=初始化.预生成文本(动画包显示支持数组[动画显示页码],false,[40,30,100])
