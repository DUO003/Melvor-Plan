...
func 克隆配方节点(配方列表=["铁锭", "纤维", "鞣革"], 原始节点=原始配方节点):# 根据配方列表数量克隆节点
	...
	for i in range(配方列表.size()):
		...
		if 配方名标签 != null and 配方名标签 is Label:
			if 配方解锁(配方名称):
				配方名标签.text = 配方列表[i]
				var 配方等级 = 计划.梅存档.get("手工",{}).get(配方名称, 0)
				var 残缺配方 = 计划.梅存档.get("手工",{}).get(配方名称+"碎片", 0)
				if 残缺配方==0:
					克隆节点.text=""
				else:
					if 配方等级==0:
...
func 鼠标点击(配方序号,按键信号):
	...
	if 按键信号.button_index == MOUSE_BUTTON_LEFT:
		...
		if 装备名称 in 制作队列:# 存在则移除
			...
			if 制作队列.size()<6:
				var 配方等级 = 计划.梅存档.get("手工",{}).get(装备名称, 0)
				if 配方等级>0:
					制作队列.append(装备名称)
					计划.语法糖通知( "已添加到制作队列:"+装备名称,"手工提示")
				else :
					var 残缺配方 = 计划.梅存档.get("手工",{}).get(装备名称+"碎片", 0)
					if 残缺配方>0:
						计划.语法糖通知( "残缺配方仅限手动制作","手工提示")
					else :
						计划.语法糖通知( "配方未解锁，无法加入队列","手工提示")
			else :
				计划.语法糖通知( "队列已满","手工提示")
...
func 制作物品检查(表格字典):
	...
	for 材料类型 in 材料类型列表:
		...
		if 所需数量 > 0:
			var 当前数量 = 计划.查看资源(材料类型)+计划.检查背包物品数量(材料类型)
