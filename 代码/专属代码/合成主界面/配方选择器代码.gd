...
# 鼠标进入事件处理方法
func 鼠标进入(配方序号:int):
	if not 计划.表格.创世蓝图.size()>配方序号:
		return
	信号序号=配方序号
	var 配方名称 = 计划.表格.创世蓝图[配方序号][0]
	#var 文本:String=计划.预生成文本(配方序号,true,[30,25,-1])+"\r右键加入或移除制作队列"
	#计划.全局悬浮提示.emit(文本,self,30)
	var 提示数据:梅提示数据=梅提示数据.new()
	提示数据.节点=self
	提示数据.默认字体=30
	提示数据.标题高度=50
	提示数据.生成蓝图(配方名称)
	提示数据.通用提示(0,"",{"样式":{"制作":["鼠标左键"],"加入队列":["鼠标右键"]}})
	计划.数据包提示.emit(提示数据)
# 鼠标离开事件处理方法
func 鼠标离开(配方序号:int):
	if 信号序号==配方序号:
