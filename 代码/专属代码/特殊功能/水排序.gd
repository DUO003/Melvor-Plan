...
func 计算奖励() -> String:
	...
	var 小游戏奖池 = [
		{"名称": "蓝图纸","数量公式": func(): return 3 ** min(3, X) * 20},
		{"名称": "金币","数量公式": func(): return 3 ** min(5, X) * 500},
		{"名称": "绿色电路板","数量公式": func(): return 10*X+25},
		{"名称": "颜料瓶","数量公式": func(): return randi_range(2*X+5,2)}]
	# 3. 执行X次额外抽取
	for i in range(max(1, X-2)):
		# 随机抽取一个奖池内容
