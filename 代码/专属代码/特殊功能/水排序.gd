...
[img=40x30]res://素材/游戏素材/货币/17.png[/img]本关卡门票0点过期
难度0:[url=1]颜料罐[/url]*4
...
func 游戏胜利():
	...
	if 计划.体力门票(0,"水排序"):
		计算奖励()
		奖励="恭喜通关:完成难度"+str(["一", "二", "三", "四", "五"][奖励结算关卡-1])+"\r"
	%关卡内容.text=奖励+固定文本
	%"关卡容器".visible=true
	清空瓶子()
...
func 计算奖励():
	计划.steam.解锁成就("颜料排序")
	var 获得物品=[]
	计划.梅存档["金币"]+=1000+200*奖励结算关卡
	var 金币=标准物品.new(1,"金币")
	金币.current_amount=1000+200*奖励结算关卡
	获得物品.append(金币)
	获得物品.append(计划.语法糖获得物品("颜料瓶",奖励结算关卡))
	if randf()>0.1+0.01*奖励结算关卡:
		获得物品.append(计划.语法糖获得物品("挂机精通代币",2))
