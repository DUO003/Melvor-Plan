...
@export var 循环跳转:String=""
var 跳转对应关系:Dictionary={
	"无进度":"","计时":"设置界面","合成":"合成界面",
	"研究":"合成_抽奖机界面",
	"炼金":"炼金界面","烹饪":"烹饪界面","订单":"订单界面",
	"预制":"烹饪界面","消费":"背包界面"}
...
func 设置暂停状态() -> void:
	if 任务状态:
		...
		if 剩余时间>0:
			...
			剩余时间 = -1
func 跳转():
	if not 循环跳转=="" and 跳转对应关系.has(循环跳转):
		计划.切换场景(跳转对应关系[循环跳转])
	else :
		计划.语法糖通知("错误,缺少跳转参数,可能是旧版本创建的任务")
