...
func 物品数量判断()->bool:
	...
	var 物品数量=0
	var 标签=计划.表格.缓存蓝图标签[道具名称]
	if 特殊标签=="":
		物品数量=计划.检查背包物品数量(道具名称)
		#print("道具名称",道具名称,物品数量)
	elif 标签=="食材":物品数量=计划.手工.烹饪预制(道具名称,特殊标签)
	return 物品数量>=当前值
func 鼠标信号处理(鼠标信号):
	#print(鼠标信号)
