...
@export var 半径: float = 40:# 半径大小（修改时自动更新圆心和最小尺寸）
	set(值):
		# 限制半径为正数
		var 新半径 = max(值, 线条宽度)  # 最小 线条宽度 像素避免显示异常
		半径 = 新半径
		queue_redraw()
@export var 起始角: float = 0:  # 开始角度（修改时自动重绘）
	set(值):
		# 限制值在0-360度范围
		起始角 = clamp(值, 0, 360)
...
var 结束角度# 结束角度（修改时自动重绘）
@export var 进度: float = 0.1:# 进度值（0-1）
	set(值):
		进度 = clamp(值, 0.0, 1.0)
		queue_redraw()
@export var 线条颜色: Color = Color(0, 0.8, 1):# 线条颜色（修改时自动重绘）
	set(值):
		线条颜色 = 值
		queue_redraw()
@export var 线条宽度: float = 10.0:# 线条宽度（修改时自动重绘）
	set(值):
		线条宽度 = 值
		queue_redraw()
@export var 背景颜色: Color = Color(0.2, 0.2, 0.2):# 背景颜色（修改时自动重绘）
	set(值):
		背景颜色 = 值
		queue_redraw()
var 进度缓动: float=进度# 进度值（0-1）
func _draw():
	if not 进度条显示:
		return
	结束角度 = 开始角度 + PI * 2
