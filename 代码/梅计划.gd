...
func 初始化存档():
	...
	if not 梅存档.has("金币"):
		梅存档["金币"]=0
	字典结构(梅存档["挂机"],
	["任务数据","任务进度","体力","在线时间","背包与商店","装备栏","快速移动关系","窗口","随身商店","订单","用户信息","原罪数据"],
	["窗口禁用","窗口解锁","窗口禁用","红点存档","全局图钉",])
...
func 预生成文本(配方编号,配方解锁=false,富文本支持: Array[int]=[-1],_项目类型="合成"):
	...
	for 键 in 手工.资源字典.keys():
		if 表格字典.has(键) and 表格字典[键] > 0:
			var 材料数量:float=float(表格字典[键])
			if 配方升星>=1:
				材料数量=材料数量*0.9
				材料列表.append(键 + "*%.1f"%材料数量)
			else :材料列表.append(键 + "*%.0f"%材料数量)
	var 制作耗时:float=float(表格.蓝图数据(配方名称,"冷却"))
	var 内容字符串 = "费用:"+"%.1f秒"%制作耗时 + ("\r"+"\r".join(材料列表) if 材料列表.size() > 0 else "无")
	...
	if 配方解锁:
		...
		if 富文本支持==[-1]:内容字符串=表格字典["简介"]+"\r"+内容字符串
		else :
...
##封装的精通获取逻辑
func 语法糖_快速熟练精通(系统,项目=null,熟练收益=true,类型="合成"):
	var 时间戳路径=梅存档[系统]
...
func 语法糖强调通知(文本:String,替换标签:String,标签点击动作:Callable=func():pass):
...
func 数据订单(返回="订单上限",增加量=0,订单类:int=0):
	...
	match 返回:
		"订单上限":
			var 标准订单上限 = [3, 0]
			if not 订单.get(返回, null) is Array:
				订单[返回] = 标准订单上限
			if 订单[返回].size()<标准订单上限.size():
				var 最终订单上限 = []
				for 索引 in range(标准订单上限.size()):
					if 索引 < 订单[返回].size():
						最终订单上限.append(订单[返回][索引])
					else:
		...
		"订单存档":
			...
			if 增加量 is Dictionary:
				订单[返回]+=[增加量.duplicate(true)]
			elif 增加量 is String:#ID的值类型为String#销毁订单的凭据,通过增加量传入
				订单[返回] = 订单[返回].filter(func(订单数据): return 订单数据.get("ID") != 增加量)
				订单[返回] = 订单.get(返回, null)
			return 订单[返回]
		_:
			return -1
#endregion 订单功能
