...
func 正式加载() -> void:
	...
	创建计时器(1.0, func():
		...
		在线时间更新())#更新在线时间
	今日零点=获取零点时间戳()
	创建计时器(获取零点时间戳()-Time.get_unix_time_from_system(),
	Callable(self, "零点检测"),{"是否循环"=false, "回传计时器"=true})# 封装计时器方法每5秒 1.检查门票过期,研究费用重置
	创建或更新商店刷新计时器()
...
func 预生成文本(配方编号,配方解锁=false,富文本支持: Array[int]=[-1],_项目类型="合成"):
	...
	if 配方解锁:
		...
		if 富文本支持==[-1]:内容字符串=表格字典["简介"]+"\n"+内容字符串
		else :
			内容字符串="[font_size="+str(富文本支持[1])+"]"+表格字典["简介"]+"[/font_size]"+"\n"+内容字符串
