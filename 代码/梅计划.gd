...
func 正式加载() -> void:
	...
	梅BUFF单例=附加代码("梅BUFF")#玩法计算数据需要先加载BUFF以防错误
	BUFF.初始化BUFF()
	梅手工单例=附加代码("梅手工")
	if 系统解锁("手工"):
		手工.手工系统上线()
...
func 初始化存档():
	...
	"窗口","随身商店","订单","用户信息","原罪数据","技能树","状态","队列"],
...
func 预生成文本(配方,配方解锁=false,富文本支持: Array[int]=[-1],_项目类型="合成")->String:
	...
	if 配方解锁:
		var 缓存字符串
		if 配方等级>0:
			缓存字符串="LV:"+str(配方等级)+" ("+str(手工.数据合成配方(配方名称,"精通"))+"/"+str(结算升级("手工","合成",配方名称,true))+")\r"
...
func 处理订单更新()->int:
	...
	if 需增加订单次数 > 0:
		var 订单数量:float=需增加订单次数*计划.数据技能树("额外的订单")+randf()
...
func 数据原罪(返回="熟练转移",项目="手工",修改=null):
	...
	match 返回:
		"熟练转移":
			...
			if 修改 is float:# 修改逻辑：仅当修改为浮点数时覆盖，限制范围0~1
...
func 数据精通上限(系统):
	...
	if 系统=="挂机":
		return 5000+等级*800+数据技能树(系统+"核心")
	if 等级==-1:
		return 10000
	return 10000+等级*1000+数据技能树(系统+"核心")
...
func 数据技能树(技能="技能名称",返回="强度",增值=0):
	...
	match 返回:
		...
		"最大等级":return 技能树字典[技能].get(返回,0)
		"基础强度":return 技能树字典[技能].get(返回,0)
		"每级增加强度":return 技能树字典[技能].get(返回,0)
		"前置技能":return 技能树字典[技能].get(返回,[])
		"图标代码":return 表格.道具贴图(技能树字典[技能].get(返回,"未知贴图"))
	var 系统=技能树字典[技能].get("系统","挂机")
	if 梅存档.has(系统) and 梅存档[系统].has("技能树"):
		var 技能树:Dictionary=梅存档[系统]["技能树"]
		...
		match 返回:
			...
			"强度":return 技能树字典[技能]["基础强度"]+(技能等级*技能树字典[技能]["每级增加强度"])
			"研究费用":
...
func 检查技能树队列()->bool:
	var 技能: Dictionary=技能队列参数("技能")
	if 技能["制作队列"].size()==0:return false
	技能数组合法性检查(技能,"制作队列")
	...
	for 技能字典 in 技能["制作队列"]:
		if 当前时间>=技能字典["时间戳"] and 技能字典["制作次数"]>=1:
			...
			if 当前等级>=最大等级:#超过最大等级
				...
				var 金币限制=false
				if 研究费用*研究推进>梅存档["金币"]:
					研究推进=int(1.0*梅存档["金币"]/研究费用)
					金币限制=true
					if 研究推进==0:
						语法糖通知("金币不足")
						技能["制作队列"]=[]
						return true
				if 语法糖金币消费(研究费用*研究推进):
					修改=true
					var 新进度:float=数据技能树(技能字典["技能名称"],"研究进度",研究推进*研究强度)
					if 新进度<=当前进度:
						技能字典["制作次数"]+=-1
