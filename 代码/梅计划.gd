...
func 初始化存档():
	...
	"窗口","随身商店","订单","用户信息","原罪数据","技能树","状态","队列","成就","收藏品","升级检查","标准商店","背包数据","地图"],
...
func 获得物品语法糖(物品名称, 数量=1,类型="标准物品",_参数 = null):
	...
	return 道具
func 点数处理(道具:标准物品):
	var 道具名称=道具.item_name
	手工.获得资源(道具名称,道具.current_amount)
	return
##返回背包内物品数量
...
func 刷新订单(订单存档:Array=[],强制:bool=true)->Array:
	...
	if 订单配置 is Array:
		var 挂机等级=数据系统("挂机","等级")
		if 订单配置.size()>=2 and 挂机等级>=5:
			订单配置[1]+=int(挂机等级/10.0)
	for 订单 in 订单存档:
		if 订单 is 梅订单数据:
			if 强制 or 订单.名称=="":
				订单.生成订单数据()
	...
	for i in range(订单类型.size()):
		var 设置数量=订单配置[i]
		if not 数据订单("订单解锁",订单类型[i]) and 设置数量>0:
			设置数量=0
		if 设置数量>0:
			for _i in range(设置数量):
