...
func 语法糖_快速熟练精通(系统,项目=null,熟练收益=true,类型="合成"):
	...
	if 收益 > 0 and 熟练收益:
		...
		var 精通= 数据.get("精通",0)+收益
		梅存档[系统]["熟练"]=熟练
		if 梅存档[系统]["精通"] < 数据精通上限(系统):
			梅存档[系统]["精通"]=精通#允许少量超出上限
		else :
			梅存档[系统]["精通"]=int(精通*0.5)#超出上限部分收益减半
		计划.语法糖通知(系统+"系统熟练+"+str(收益)+"精通池(+"+str(int(收益/2.0))+")",系统+"熟练")
		结算升级(系统)
	if 项目!=null:
		收益=0
		match 类型:
	...
	match 返回:
		"熟练转移":
			...
			if 修改 is float:# 修改逻辑：仅当修改为浮点数时覆盖，限制范围0~1
				原罪数据["熟练转移"][项目] = clamp(修改, 0.0, 1.0)
			return float(原罪数据[返回].get(项目, 0.0))
		"原罪值":
			if not 原罪数据.has(返回):原罪数据[返回]={}
			if 修改 is int:# 修改逻辑：仅当修改为整数时增加
				原罪数据["熟练转移"][项目] = clampi(int(修改+原罪数据[返回].get(项目, 0)),0,数据原罪("原罪上限",项目))
			return int(原罪数据[返回].get(项目, 0))
		"原罪上限":
			if not 原罪数据.has(返回):原罪数据[返回]={}
			if 修改 is int:# 修改逻辑：仅当修改为整数时增加
				原罪数据["熟练转移"][项目] = int(修改+原罪数据[返回].get(项目, 0))
